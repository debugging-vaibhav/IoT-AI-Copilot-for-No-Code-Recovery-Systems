  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>
        ...
        {device.uptime && <InfoRow icon={<UptimeIcon />} text={`Uptime: ${device.uptime}`} />}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
        {device.status === "offline" && (
          <Button variant="dark" onClick={() => onAction(device.id, "recover")} style={{ flex: 1 }}>
            Auto-Recover
          </Button>
        )}
        {device.status === "warning" && (
          <Button variant="outline" onClick={() => onAction(device.id, "diagnose")} style={{ flex: 1 }}>
            Run Diagnostics
          </Button>
        )}
        <Button variant="outline" onClick={() => onAction(device.id, "configure")} style={{ flex: 1 }}>
          Configure
        </Button>
      </div>
    </div>
