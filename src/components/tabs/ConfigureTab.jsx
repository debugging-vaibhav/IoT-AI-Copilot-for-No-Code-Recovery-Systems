...
import { applyRecovery, generateLogic } from "../../data/apiClient";
...
const inputStyle = {
  ...
  fontSize: 13, outline: "none", boxSizing: "border-box", background: "#f9fafb", fontFamily: "inherit",
};
...
export function ConfigureTab({ components, setComponents, showToast }) {
  ...
  const handleGenerateLogic = async () => {
    setGenerating(true);
    try {
      const description = `${missionObjective}. ${controlBehavior}. Components: ${components.map((c) => `${c.name} on ${c.pin}`).join(", ")}`;
      const logic = await generateLogic(description);
      setGeneratedLogic(logic);
      setSummary([
        `Read ${logic.sensor} data via configured protocol`,
        `Rule: ${logic.rule}`,
        `Action: Set Pin ${logic.pin} to ${logic.action}`,
        "Monitor system state and apply recovery logic on failure",
      ]);
      showToast("Control logic generated from backend!", "info");
    } catch (err) {
      // Fallback to local
      const sensor = components.find((c) => c.type === "Sensor");
      const actuator = components.find((c) => c.type === "Actuator");
      const fallbackLogic = {
        sensor: sensor?.name?.toLowerCase() || "generic_sensor",
        pin: 17,
        action: "ON",
        rule: `IF ${sensor?.name || "sensor"}_value > threshold THEN ON`,
      };
      setGeneratedLogic(fallbackLogic);
      setSummary([
      ...
      showToast("Used local logic generation (backend unavailable)", "info");
    }
    setGenerating(false);
  };
  ...
  const handleDeploy = async () => {
    if (!generatedLogic) {
      showToast("Generate control logic first!", "error");
      return;
    }
    setDeploying(true);
    try {
      const result = await applyRecovery(generatedLogic);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <FormField label="Component Name" value={form.name} onChange={(v) => setForm((f) => ({ ...f, name: v }))} placeholder="e.g., LiDAR Module, GPS Sensor" />
              <div>
                <label style={labelStyle}>Component Type</label>
                <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} style={inputStyle}>
                  <option>Sensor</option><option>Actuator</option><option>Controller</option><option>Communication</option>
                </select>
              </div>
              <FormField label="Pin Configuration" value={form.pin} onChange={(v) => setForm((f) => ({ ...f, pin: v }))} placeholder="e.g., GPIO 14, 15 or PWM 5-8" />
              <div>
                <label style={labelStyle}>Component Description</label>
                <textarea value={form.desc} onChange={(e) => setForm((f) => ({ ...f, desc: e.target.value }))} placeholder="Describe what this component does" style={{ ...inputStyle, minHeight: 80, resize: "vertical" }} />
              </div>
              <button onClick={addComponent} style={{
                padding: "12px 0", background: "#111827", color: "#fff", border: "none", borderRadius: 8,
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {components.length === 0 && (
                <div style={{ color: "#9ca3af", fontSize: 13, textAlign: "center", padding: 20 }}>No components added yet</div>
              )}
              {components.map((c) => (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2" style={{ marginTop: 2 }}>
                        <rect x="7" y="7" width="10" height="10" rx="1" /><path d="M7 12H4m13 0h3M12 7V4m0 13v3" />
                      </svg>
                      <div>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span style={{ background: "#f3f4f6", color: "#374151", padding: "2px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>{c.type}</span>
                      <button onClick={() => removeComponent(c.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, display: "flex" }}>
                        <TrashIcon size={16} color="#9ca3af" />
                      </button>
                    </div>
                  </div>
                  {c.desc && <div style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>{c.desc}</div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24 }}>
            ...
            <textarea value={missionObjective} onChange={(e) => setMissionObjective(e.target.value)} style={{ ...inputStyle, minHeight: 90, resize: "vertical" }} />
            <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>Explain what the robot should accomplish in the field</div>
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24 }}>
            ...
            <textarea value={controlBehavior} onChange={(e) => setControlBehavior(e.target.value)} style={{ ...inputStyle, minHeight: 90, resize: "vertical" }} />
            <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>Explain how components should interact and respond</div>
          </div>
          ...
          <div style={{ background: "#f8f7ff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              ...
              <button
                ...
                disabled={deploying || !generatedLogic}
                style={{
                  flex: 1, padding: "11px 0",
                  background: deploying ? "#22c55e" : "#fff",
                  color: deploying ? "#fff" : "#111827",
                  border: deploying ? "none" : "1.5px solid #d1d5db",
                  borderRadius: 8, fontWeight: 700, fontSize: 13,
                  cursor: "pointer", fontFamily: "inherit",
                  opacity: !generatedLogic ? 0.5 : 1,
                }}
              >
                {deploying ? "Deploying..." : "Deploy to RPi"}
              </button>
            </div>
          </div>
        </div>
