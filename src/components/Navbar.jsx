...
function ConnectionDot({ status }) {
  const color =
    status === "online" ? "#22c55e" : status === "checking" ? "#f59e0b" : "#ef4444";
  return (
    <span
      style={{
        width: 8,
...
export function Topbar({ backendStatus, connectedDevices }) {
  const rpiCount = Object.values(connectedDevices || {}).filter(
    (d) => d.status === "ONLINE"
  ).length;
    ...
    >
      ...
      <div style={{ position: "absolute", right: 24, display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9ca3af" }}>
          <ConnectionDot status={backendStatus} />
          Backend
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#9ca3af" }}>
          <ConnectionDot status={rpiCount > 0 ? "online" : "offline"} />
          RPi ({rpiCount})
        </div>
      </div>
    </div>
...
export function AppHeader({ backendStatus, connectedDevices }) {
  const rpiCount = Object.values(connectedDevices || {}).filter(
    (d) => d.status === "ONLINE"
  ).length;
  const systemActive = backendStatus === "online" && rpiCount > 0;
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "#6b7280" }}>
            No-Code Recovery &amp; Control System
          </p>
        </div>
      </div>
      ...
      <div
        style={{
          background: systemActive ? "#f0fdf4" : backendStatus === "online" ? "#fffbeb" : "#fef2f2",
          border: `1.5px solid ${systemActive ? "#86efac" : backendStatus === "online" ? "#fde68a" : "#fca5a5"}`,
          color: systemActive ? "#15803d" : backendStatus === "online" ? "#92400e" : "#dc2626",
          padding: "6px 16px",
          borderRadius: 20,
          fontWeight: 700,
          fontSize: 13,
